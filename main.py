...
review_llm_chain = ai_agent.init_review_agent() 
...
report_type = st.selectbox(
    ...
    ["Difficulty Report", "Weekly Market Update", "Weekly Bitcoin Recap"],
)
...
]:  # Check for either report that requires news input
...
if report_type == "Weekly Bitcoin Recap":
    weekly_bitcoin_chart = st.file_uploader(
        "Upload Weekly Bitcoin Chart (.png)", type=["png"]
    )
    ...
    if report_type == "Difficulty Report":
        ...
        if weekly_bitcoin_chart:
            ...
            report = weekly_bitcoin_recap.generate_full_report(
                chat_llm_chain, review_llm_chain, news_stories, report_date, weekly_bitcoin_chart
            )
